...
const mapUser = (p: WykopAuthor): User => ({
  login: p.login,
  status: 1, // p.color,
  ...(p.avatar !== WYKOP_DEFAULT_AVATAR_URL && { avatarUrl: getOptimizedAvatarUrl(p.avatar) }),
  ...(p.sex && { sex: p.sex }),
});
...
export const mapEntry = (e: WykopEntry): Entry => ({
  ...
  ...(e.comments?.length && { comments: mapEntryComments(e.comments) }),
});
...
export const mapLink = (l: WykopLink): Link => ({
  ...
  isHot: l.is_hot,
  plus18: l.plus18,
  ...(l.preview && { previewUrl: l.preview }),
  relatedCount: l.related_count,
  sourceUrl: l.source_url,
  title: l.title,
  ...(l.comments?.length && { comments: mapLinkComments(l.comments) }),
});
